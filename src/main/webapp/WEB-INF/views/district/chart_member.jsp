                                                ...
                                                <div class="row justify-content-center">
													<ul class="nav nav-pills nav-justified" role="tablist">
														<c:forEach var="item" items="${districts}" varStatus="status">
															<c:if test="${gunCode == item.districtName}">
																<button type="button" class="btn btn-info waves-effect waves-light" style="border-width:20px; border-color:yellow; margin-bottom: 20px; border-bottom-right-radius: 0; border-top-right-radius: 0;" onclick="clickGun(${item.cityCode},${item.gunCode})">${item.districtName}<br>${item.districtCount}</button>
															</c:if>
															<c:if test="${gunCode != item.districtName}">
																<button type="button" class="btn btn-info waves-effect waves-light" style="margin-bottom: 20px; border-bottom-right-radius: 0; border-top-right-radius: 0;" onclick="clickGun(${item.cityCode},${item.gunCode})">${item.districtName}<br>${item.districtCount}</button>
															</c:if> 
